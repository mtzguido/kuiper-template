...
# Install the exact formatter used to canonicalize generated CUDA.
set -euo pipefail
...
mkdir -p "$dest/bin"
install -m 755 \
  "$workdir/unpacked/clang_format/data/bin/clang-format" \
  "$dest/bin/clang-format"

actual=$("$dest/bin/clang-format" --version)
[[ "$actual" == "clang-format version $version" ]] || {
  echo "Error: formatter reports '$actual'." >&2
  exit 1
}
